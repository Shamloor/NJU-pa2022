...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = imm);
	...
	INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb		 , S, Mw(src1 + imm, 1, (int8_t)(src2 & 0xFF)));
	INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh		 , S, Mw(src1 + imm, 2, (int16_t)(src2 & 0xFFFF)));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
	...
	INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai	 , I, R(rd) = (src1 >> (imm & 0x1F)) | ((src1 & 0x80000000) ? ~(0xFFFFFFFF >> (imm & 0x1F)) : 0));
	...
	INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra		 , R, R(rd) = src1 >> (src2 & 0x1F) | ((src1 & 0x80000000) ? ~(0xFFFFFFFF >> (src2 & 0x1F)) : 0));
	INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or		 , R, R(rd) = src1 | src2);
	INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and		 , R, R(rd) = src1 & src2);
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
	
	INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul		 , R, R(rd) = src1 * src2);
	INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh	 , R, int64_t tmp = (int64_t)(int32_t)src1 * (int64_t)(int32_t)src2; R(rd) = tmp >> 32);
	//INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = ((uint64_t)src1 * (uint64_t)src2) >> 32);
	//INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu	 , R, R(rd) = ((int64_t)src1 * (uint64_t)src2) >> 32);
